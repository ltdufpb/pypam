...
def clean_state():
    ...
    if os.path.exists(ALLOWLIST_FILE): shutil.copy(ALLOWLIST_FILE, backup_allowlist)
    if os.path.exists(ADMIN_CREDS_FILE): shutil.copy(ADMIN_CREDS_FILE, backup_admin)
    
    # Create fresh files
    with open(ALLOWLIST_FILE, "w") as f: f.write("testuser:testpass\n")
    with open(ADMIN_CREDS_FILE, "w") as f: f.write("admin:admin123\n")
...
async def test_student_login():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        ...
        # Invalid login
        response = await ac.post("/login", json={"username": "testuser", "password": "wrong"})
        assert response.status_code == 200
...
async def test_admin_login():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        response = await ac.post("/admin/login", json={"username": "admin", "password": "admin123"})
        assert response.status_code == 200
        assert response.json() == {"success": True}
